import { AccountRole, Match } from '@prisma/client';
...
describe('Update Match Controller (e2e)', () => {
  ...
  describe('admin operations', () => {
    beforeAll(async () => {
      await usersRepository.update(userId, { role: AccountRole.ADMIN });
    });

    afterAll(async () => {
      await usersRepository.update(userId, { role: AccountRole.USER });
    });
    ...
    it('should be able to update a match', async () => {
      ...
      updatedMatchDate.setDate(updatedMatchDate.getDate() + 1);
